...
class ScrollComponent extends Component {
  constructor() {
    ...
    axios
      .get(
        `https://sf-legacy-api.now.sh/items?page=${page}`
      ).then(res=>res.data)
      .then(res => {
	...
	handleClickItem = (item) => {
		...
		localStorage.setItem('app_selected_items', JSON.stringify(this.state.selectedItems));
	}
      ...
      <div className="container">
        <ul style={{ minHeight: "800px" }}>
          {items.map(item => {
						...
						return (<li key={item.absoluteIndex} className={selectedClass} onClick={()=>this.handleClickItem(item)}>
							<span className={`item-title ${selectedClass}`}>{item.name}</span>
							<span className={`item-id ${selectedClass}`}>{item.id}</span>
							<span className={`item-index ${selectedClass}`}>[ {item.absoluteIndex} ]</span>
						</li>);
					})}
        </ul>
        <div
